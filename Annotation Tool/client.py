            ...
            if mainWin.params["keyPoints"][idx]:
                ...
                for key in cat["keypoints"]:
                    ...
                    if kpKey in mainWin.params["keyPoints"][idx]:
                        coor = utils.mult(mainWin.params["keyPoints"][idx][kpKey][::-1],frame.shape[:2])
...
class optFlowWindow(QDialog, optFlow_Ui_Dialog):
    ...
    def continue_(self):
        ...
        for idx in range(self.cur, end, direct):
            print(idx,end='\r')

            offset = int(direct<0)
            if _optFlow[idx-offset].sum()==0:
                print('send request')
                prev = cv.imencode('.jpg',mainWin.videoCap[idx])[1].tobytes()
                next_ = cv.imencode('.jpg',mainWin.videoCap[idx+direct])[1].tobytes()
                try:
                    ...
                    if response.status:
                        flo = np.frombuffer(response.forw,dtype='float16').reshape((H,W,2))
                    else:
                ...
                flo = _optFlow[idx-offset]
            cur_kps = mainWin.params["keyPoints"][idx]
            next_kps = mainWin.params["keyPoints"][idx+direct]
...
class configWindow(QDialog,config_Ui_Dialog):
    def __init__(self, mainWin, parent=None):
        ...
        self.Video.clicked.connect(self.ChangeVideo)
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    def __init__ (self, parent=None):
        ...
        self.pb_CalcOptFlow.clicked.connect(self.calcOptFlow)
        ...
        ## Slider ##
        self.horizontalScrollBar.valueChanged.connect(self.slide)
...
###### STATUS ######
    def resetWrapper(self):
        ...
        self.horizontalScrollBar.setMaximum(self.maxpage-1)
        self.params["page"] = temp
        self.horizontalScrollBar.setSliderPosition(self.params["page"])
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            ...
            self.horizontalScrollBar.setValue(self.params["page"])
...
###### ALGORITHM ######
    #Connection with Openpose
    def Openpose(self):
        if not self.videoCap:
            reply = QMessageBox.critical(self, "Error", "Empty Image Collection!!", QMessageBox.Ok, QMessageBox.Ok)
            return
        
        self.printMsg('Waiting for key points from Openpose Server...')
        try:
            ...
            # self.connectionTimer.stop()
            kp = eval(response.result.decode(encoding='utf-8'))
            for key in kp:
                self.params["keyPoints"][cursor][key] = kp[key]
            self.updateWinTitle(False)
            msg = self.params["keyPoints"][cursor]
    ...
    def calcOptFlow(self):
        if not self.videoCap:
            reply = QMessageBox.critical(self, "Error", "Empty Image Collection!!", QMessageBox.Ok, QMessageBox.Ok)
            return

        assert isinstance(self.params["path"],str)
        MAX_MESSAGE_LENGTH = 1024*1024*1024
        videoName = self.params["path"].split('/')[-1]
        try:
            dlg = QFileDialog()
            dlg.setFileMode(QFileDialog.Directory)
            ...
            with grpc.insecure_channel(self.IP['RAFT'],options=[
                ...
                if response.status:
                    ...
                    self.RAFT_FW = writeReadRAFT(path+'/forw.avi', forw)
                    self.RAFT_BW = writeReadRAFT(path+'/backw.avi', backw)
                    if utils.checkRAFT(self.videoCap, self.RAFT_FW, self.RAFT_BW):
                        ...
                        self.RAFT_FW = None
                        self.RAFT_BW = None
                        os.remove(path+'/forw.avi')
                        os.remove(path+'/backw.avi')
...
###### DISPLAY ######
    ...
    def slide(self):
        self.params["page"] = self.horizontalScrollBar.sliderPosition()
        self.sb_Page.setValue(self.params["page"]+1)
        self.loadKeyPointList()
        self.update()
    ...
    def spinBox(self):
        self.params["page"] = self.sb_Page.value()-1
        self.horizontalScrollBar.setValue(self.params["page"]) #会触发 self.slide, 不需要做update. (spinBox基本上和slider绑定在一起)
